...
def test_unknown_keys_are_dropped_and_order_is_canonical() -> None:
    parsed = parse_collapsed("waiting,nope,assigned.due.upcoming.completed")
    assert parsed == frozenset(
        {"waiting", "assigned", "due", "upcoming", "completed"},
    )
    assert encode_collapsed(parsed) == "due.upcoming.assigned.waiting.completed"
