...
Filters on the list endpoint: `type`, `status`, `assignee_id`, `sprint_id`, `parent_id`, `unscheduled` (a boolean selecting issues with no sprint), `label` (a normalized name), and `unlabeled` (issues with no labels).
...
`rollup` covers the issue and all its descendants ([ADR 012](../adr/ADR-012.md)); the issue's own `estimate_minutes` is never overwritten by it. Progress counts *Done* descendants separately from *Cancelled* ones ([ADR 020](../adr/ADR-020.md)). Moving a card on the board is a `PATCH` of `status`; moving an issue into or out of a sprint is a `PATCH` of `sprint_id`. `labels` is a list of names; a `PATCH` that names `labels` replaces the set, and `[]` clears it ([ADR 028](../adr/ADR-028.md)).
