...
* Enumerated values on the wire are the stored strings: types `epic`, `story`, `subtask`; statuses `todo`, `in_progress`, `in_review`, `blocked`, `done`, `cancelled`; sprint states `planned`, `active`, `completed`; sprint cadences `off`, `weekly`, `two_weeks`, `monthly`, `every_n_days`; series states `active`, `paused`, `stopped`; spawn modes `calendar`, `after_closed`; sprint bases `due_on`, `start_on`, `created_on`; recurrence `daily`, `weekly`, `monthly`, `yearly`; dependency kinds `blocks`, `relates_to`.
...
Issues spawned from a series include `series_id` and `occurrence_on`. Each series stores start and due as day offsets plus minutes-of-day from the occurrence date, and `sprint_basis` may be `due_on`, `start_on`, or `created_on` ([ADR 021](../adr/ADR-021.md), [ADR 023](../adr/ADR-023.md)).
