...
* **Separate the board by sprint.** A user stacks a row of columns per sprint so cards sit with the sprint they belong to. On the master board those rows are named with the project key, and dropping a card onto another project's row is refused.
