...
> **Amendment.** [Completed sprints off the board](completed-sprints-off-the-board.md) applies this ADR's completed-sprint hide rule to the project board as well. A completed sprint is not a Kanban lane unless it still has open work, and it is not listed in the board's sprint filter. The requirement below that leaves `/projects/{key}/board` unchanged, and the disagreement in Consequences, are superseded by that record.
...
* **Must** open Separate by sprint by default, matching the project board. **Must** prefix each sprint lane title with the project key when it is on. **May** turn Separate by sprint off for one shared row of columns.
