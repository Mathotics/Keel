...
> **Amendment.** [ADR 020](ADR-020.md) keeps unresolved blockers as those whose status is not *Done*; *Cancelled* does not clear a blocker.
...
* **Must** show on board cards and backlog rows a marker counting direct blockers that are not *Done*.
...
Blocker counts for a view come from one aggregate query over dependencies joined to issues, counting *blocks* links whose source is not *Done*, grouped by target. The board and backlog services attach the counts to their rows, so rendering costs one additional query rather than one per card.
