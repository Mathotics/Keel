...
* **Must** define exactly six ordered statuses: *To Do*, *In Progress*, *In Review*, *Blocked*, *Done*, *Cancelled*.
* **Must** treat *Done* and *Cancelled* as closed statuses, used to decide backlog membership and sprint carry-over.
* **Must** treat only *Done* as completed, used to decide blocker resolution and descendants-done progress, per [ADR 020](ADR-020.md).
...
* **Must** define the backlog as the project's issues that have no sprint assigned and are not closed.
...
* **Must** define an unfinished issue as one whose status is not closed.
...
* **Must** leave closed issues attached to the completed sprint as a record of what it held.
...
```mermaid
flowchart TD
  complete["Complete active sprint"] --> select["Select its issues that are not closed"]
