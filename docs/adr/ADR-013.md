...
- Board composition, including a type filter.
...
- Swimlanes, work-in-progress limits, and board filters beyond issue type.
...
`keel.services.boards` builds the board projection with one query per render — issues for the project, filtered by type, grouped by status in Python — plus one aggregate query for unresolved blocker counts. `keel.services.backlog` applies the membership filter and creation-time ordering.
...
flowchart TD
  ...
  board["Board - every type by default, filterable, grouped by status"]
