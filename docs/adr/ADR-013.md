...
- Board composition, including type and assignee filters.
...
- Swimlanes, work-in-progress limits, and board filters beyond type and assignee.
...
`keel.services.boards` builds the board projection with one query per render — issues for the project, filtered by type and assignee, grouped by status in Python — plus one aggregate query for unresolved blocker counts. `keel.services.backlog` applies the membership filter and creation-time ordering.
...
flowchart TD
  ...
  board["Board - every type and assignee by default, filterable, grouped by status"]
