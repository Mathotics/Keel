...
> **Amendment.** The board also filters by sprint, including unscheduled. Swimlanes, work-in-progress limits, and further filters remain out of scope.
...
- Board composition, including type, assignee, and sprint filters.
...
- Swimlanes, work-in-progress limits, and board filters beyond type, assignee, and sprint.
...
`keel.services.boards` builds the board projection with one query per render — issues for the project, filtered by type, assignee, and sprint, grouped by status in Python — plus one aggregate query for unresolved blocker counts. `keel.services.backlog` applies the membership filter and creation-time ordering.
...
flowchart TD
  ...
  board["Board - every type, assignee, and sprint by default, filterable, grouped by status"]
