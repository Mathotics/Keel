...
* **Risk:** Alembic plus SQLite cannot always roll back a half-applied revision in-engine; the file backup is the recovery mechanism. An editable install is updated in place while the old process still runs — mitigated by `KEEL_RELOAD=false` and restarting only after migrate succeeds. Self-hosted `GITHUB_TOKEN` fetch must work against the existing clone’s remotes.
...
Deploy (every successful `main` CI): `scripts/deploy-prod.sh`. The Deploy workflow checks out the CI head SHA under the runner `_work` tree only so it can run that script version; the live tree remains `/mnt/library/Keel`.
