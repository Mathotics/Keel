...
* **Must** show each row as a table in the existing list style: issue key (link to the issue), type, title, project, and a status control; **Must** show the unresolved-blocker marker when that count is non-zero.
