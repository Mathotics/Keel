...
> **Amendment.** [ADR 020](ADR-020.md) splits progress: descendants in *Done* count as done; descendants in *Cancelled* are counted separately; cancelled remaining is omitted from the parent remaining total.
...
- Progress as the count of descendant issues in *Done*, with cancelled descendants shown separately.
...
* **Must** define progress for an issue with descendants as the count of those descendants whose status is *Done*, presented as “N of M done”, with cancelled descendants counted separately when any exist.
* **Must** omit cancelled descendants’ remaining time from the rolled-up remaining total; estimates still include them.
