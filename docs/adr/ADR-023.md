# ADR 023: Issue start date/time and series start/due offsets
...
Keel issues already carry an optional due date/time. People set it on Create and on the issue page (`datetime-local`, clearable). Board cards already show due when it is set. Home lists unfinished assigned work whose due calendar date is today or earlier ([ADR 022](ADR-022.md)). Repeating series spawn copies with `due_at` at midnight on the occurrence day and assign sprints by creation date or due date ([ADR 021](ADR-021.md)). Changing due on a repeating issue currently saves that copy only and does not ask which occurrences to update. There is no way to say when work should **start**, and the scheduler cannot express a start-to-due window per cycle.
...
* Series recipe start/due offsets and start-date sprint basis on Schedules and the issue series panel.
...
* **Must** still allow This occurrence on a stopped series; **Must Not** allow recipe-wide start/due changes on a stopped series.
...
This feature extends existing product surfaces: Create (`/create`), issue page (`/issues/{key}-{number}`), board cards, backlog table, home inbox (`/`), Schedules recipe (`_series_recipe.html` on `/projects/{key}/schedules` and the issue series panel), and the JSON issue API. Start is another issue field beside due; series offsets live on the recipe people already edit.
