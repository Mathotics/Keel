# ADR 023: Lightweight per-issue field history
...
Keel is a personal tracker for one owner plus up to two collaborators. The issue page ([UI design](../design/ui.md)) shows current fields — status, assignee, sprint, estimate, remaining, due date, parent, type — and they save as soon as they change. Metadata is Created on, Updated on, reporter, and key. Comments ([ADR 017](ADR-017.md)) capture discussion when someone writes a note. Identity is the picker user ([ADR 011](ADR-011.md)).
...
- On the issue page, show a short changelog of planning-field changes: who, when, field, from → to.
...
- Lines for actual changes to **status**, **assignee**, **sprint**, **estimate**, **remaining**, **due date**, **parent**, and **type**.
...
- Title, description, created, reporter, views, failed requests, or a row per autosubmit POST that did not change a value.
...
* **Must** append a line only when an in-scope field’s value actually changes: status, assignee, sprint, estimate, remaining, due date, parent, type.
* **Must** record sprint carry-over when unfinished issues move on Complete (to the next planned sprint or to Unscheduled).
* **Must** show Unassigned, Unscheduled, No parent, and none for empty estimate/remaining/due, using the same status/type labels the issue page already uses.
...
* **Must Not** record title or description edits, create/birth values, no-op saves, failed writes, or page views.
...
* **Good:** The full audit log stays deferred; autosubmit noise (title/description) is not logged.
* **Bad:** History starts empty for issues that already exist; last week’s In Review still has no trail until the next change.
* **Bad:** Remaining-time and due-date edits can make a long list on a busy ticket; there is no collapse.
