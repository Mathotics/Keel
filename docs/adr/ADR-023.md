# ADR 023: Issue priority as a required ranked field
...
* **Must** default new issues to *P3 — Major*, including copies spawned from a series.
* **Must** backfill existing rows to *P3 — Major* when the column is added; **Must Not** leave priority nullable.
...
* **Good:** Rank is visible without opening the issue. Existing work gets a sensible middle default instead of all reading as blockers.
* **Bad:** Five labels must be learned; Blocker as P1 can be confused with the *Blocked* status.
* **Risk:** Defaulting everything to P3 hides the field until people change it. Accepted so the column can be required from day one.
* **Risk:** Series copies ignore the source issue's rank. Accepted to keep the recipe table unchanged; a later ADR can add priority to series if recipes need it.

## System Design

### Technical Stack and Architecture

`IssuePriority` lives in `keel.domain.enums` next to type and status. The ORM column, Pydantic schemas, issue service, JSON routes, and `/web` forms all pass that enumeration through. Board cards read `issue.priority` from the same board projection already used for type.

### UML Diagrams

```mermaid
flowchart LR
  create[Create issue] --> p3[Default P3 Major]
  patch[PATCH or issue page] --> rank[Stored P1 to P5]
  p3 --> observe[Board chip and list column]
  rank --> observe
```
